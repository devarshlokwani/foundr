 ...
 *
 * Scoped to the user and to a kind (expense / revenue / investment), so
 * each founder builds their own personalised lists over time. We seed a
 * few sensible defaults on first use, but from then on it's entirely
 * theirs — they can add their own and we remember them.
 *
...
const categorySchema = new Schema(
  {
    ...
    kind: { type: String, enum: ["expense", "revenue", "investment"], required: true },
